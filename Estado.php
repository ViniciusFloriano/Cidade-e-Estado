...
<body>
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="conteudoNavbarSuportado">
            ...
            <li class="nav-item active">
                <a class="nav-link" href="cadEst.php">Cadastrar estado</a>
            </li>
            <li class="nav-item">
    ...
    <form method="post">
        ...
        <button name="acao" id="acao" type="submit"  class="btn btn-outline-info">Procurar</button><br><br>
    ...
    <?php
        ...
        if($busca == 1){
            $consulta = $pdo->query("SELECT * FROM estado
                                WHERE EST_ID LIKE '$procurar%' 
                                ORDER BY EST_ID");}
        else if($busca == 2){
            $consulta = $pdo->query("SELECT * FROM estado
                                WHERE EST_NOME LIKE '$procurar%' 
                                ORDER BY EST_NOME");}
        else if($busca == 3){
            $consulta = $pdo->query("SELECT * FROM estado 
                                WHERE EST_SIGLA LIKE '$procurar%'
                                ORDER BY EST_SIGLA");}
    while ($linha = $consulta->fetch(PDO::FETCH_ASSOC)) {   
        ?>
        <tr><td><?php echo $linha['EST_ID'];?></td>
            <td><?php echo $linha['EST_NOME'];?></td>
            <td><?php echo $linha['EST_SIGLA'];?></td>
            <td><a href='cadEst.php?acao=editar&EST_ID=<?php echo $linha['EST_ID'];?>'> <img class="center" src="img/edit.png" alt=""></a></td>
            <td><?php echo " <a href=javascript:excluirRegistro('acaoEst.php?acao=excluir&EST_ID={$linha['EST_ID']}')>Excluir estado</a><br>"; ?></td>
        </tr>
    <?php } ?>       
    </table>
    </fieldset>
